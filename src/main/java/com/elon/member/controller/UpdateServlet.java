...
public class UpdateServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    	...
    	if(member != null) {//데이터가 존재하면
    		//update.jsp에서 member를 member 키로 사용할 수 있도록
    		//request 객체에 저장해줌.
    		request.setAttribute("member", member);
    	}
    		request.getRequestDispatcher("/WEB-INF/views/common/update.jsp")
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		Member member = new Member(memberId, memberPwd, email, phone, 0, address, hobby, hobby, hobby);
		MemberService mService = new MemberService();
